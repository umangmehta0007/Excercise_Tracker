...
* After signing in successfully you're able to see feed of the people you follow, add/remove follower, manage your
  current profile.
* Under managing your profile you'll be able to add activities, add/remove obstacles, add/remove gears.
* The new thing about adding activity here would be you can now choose they you want to find the Route.
* Either you can enter manually, choose from your existing activities or enter starting and ending points for your route
  system will find a route for you from either of your existing one's or the follower's list.

# Phase 3

* In this third phase, we've probably finished coding and designing our domain model and objects.
* As the phase began we made a test harness for all of our domain model object and noticed our coverage.
* As we processed further, we came across a problem that we were not able to save our data which meant:
    * Everytime I ended the program and started it again, we had to start from the starting
    * No users were saved inside, so that we can come and retrieve them.
    * All activities, routes, gears, followers were gone when we started our program.
* To fix this we introduced the persistence in this phase and we focused on another principle which was **DEPENDENCY
  INVERSION**
    * Dependencies were injected through main as a parameters to all my layers that needed them.
    * It was done because my classes don't need to know how something was done implementing our SINGLE RESPONSIBILITY
      PRINCIPLE.
* Now to save our work done, we used JSON to help us:
    * Persistence layer was seperated by interface between itself and Logic layer.
    * Logic layer was passed an instance of the persistence interface to save whenever needed.
    * The instance was constructed in main and passed as parameter to my logic layer.
    * Also, in this we used a technique known as coarse grained persistence , in which you don't specifically update
...
* pop() is incorrect as I can see it's not removing the element it has popped due to which size, peek, empty were
  failed.

* `BadStack3`
* size is of course wrong, because all other work in push.It never reported the correct number of elements once
  anything was pushed in the stack.
...
- Implementation of UI and Logic layers was done, in which UI was reponsible for Validation and Logic Layer was used for
  processing and returning Domain Model Objects.
...
- Map here has became the basic example of Single Responsibility Principle and list of activates were taken and were
  processed by Logic now.
...
- For adding activities now we have three different ways, which were choosing manually, choose from User's existing, and
  Path finding.
- For path finding we created algorithm that would ask for first and last coordinates you want and choose the way you
  want to explore the route.
...
- No changes were made to my domain model.
- All the unnecessary methods were removed.
- Testing harness was introduced for our domain model object and logic layer methods.
- Persistence was introduced and a new layer was added to our software to save data to the disk in form of Json.
...
* AI Disclaimer: All implementations and logic were self created, all logic layer to UI were created, however some
  syntax's were asked form ChatGPT like the new Switch learnt.
* I also tend to learn some understanding of Logic and UI but never shared the code, some technical and basic questions
  were asked that helped to build some stuff but never no codes were shared.
* Questions like: can we have multiple logic layers, inside logic layers, how would that affect the design and many
  more. But NO CODE/ LOGIC was taken.
...
* Computer Science help centre people have been a great help as well as my those classmates who did clear a lot of
  questions.

**CODE COPY DISCLAIMER**

* Code was directly taken from Franklin's repository (The Professor of COMP 2450)
* Test code as `TestHarness` and  `TestResults`, Code for persistence in `PersonPersistenceJson` was directly
  copy-pasted from what franklin(Professor) uploaded.
* https://www.youtube.com/@franklinbristow4964 was highly viewed to work on stuff and may highly overlap with code
  written.
* The oracle document was used as direct reference for many classes like:
    * LocalDateTime
    * Path
    * File
    * Json
...
https://docs.oracle.com/javase/8/docs/api/allclasses-noframe.html
https://javadoc.io/doc/org.glassfish/javax.json/latest/javax/json/package-summary.html
